...
class Encoder(nn.Module):
    def __init__(self,embedding_size,num_layers,num_heads,max_len,hidden,token_vocab_size,padding_index,dropout):
...
#        self.to_vocab=nn.Sequential(nn.Linear(output_size,token_vocab_size),nn.Softmax())
    ...
    def forward(self,h,h_mask,dropout=None):
        ...
        pos_emb=self.positional_embedding(h).repeat([h.shape[0],1,1])
        h=h+pos_emb
